...
class Shared(GaussianMixin, DeterministicMixin, Model):
    def __init__(self, observation_space, action_space, device,
                 ...
                 map_shape=(16, 16, 3), proprio_dim=23,  # Updated to 23
    ...
    def compute(self, inputs, role):
        ...
        # Apply preprocessing only to proprioceptive data
        if role == "policy":
            self.proprio_preprocessor.train()
        else:
            self.proprio_preprocessor.eval()
        proprio = self.proprio_preprocessor(proprio)

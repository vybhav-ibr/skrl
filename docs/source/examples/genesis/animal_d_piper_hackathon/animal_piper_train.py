...
parser.add_argument("-e", "--exp_name", type=str, default="animal-piper")
...
class MultiModalStochasticActor(GaussianMixin, Model):
    ...
    def __init__(self, observation_space, action_space, device, dropout_rate=0.2,
        ...
        # Foreground segmentation extractor (frozen)
        self.fg_extractor = YOLOForegroundExtractor(model_path="yolo11s-seg.pt")
        ...
        # print("in feature size",total_input_dim)
        # total_input_dim=169
    ...
    def compute(self, inputs, role):
        ...
        #     print("k-v pairs:",key,value.shape)
        # exit(0)
        ...
        foreground_masks = self.fg_extractor(rgb_images)  # [B, H, W]
        
        # Extract features from foreground-masked depth
        depth_features = self.depth_extractor(depth_images, foreground_masks)  # [B, 64]
        
        # print("Commands shape before view:", space["commands"].shape)
        # print("DOF Diff shape before view:", space["dof_diff"].shape)
        # print("DOF Vel shape before view:", space["dof_vel"].shape)
        # print("Object Pos shape before view:", space["object_pos"].shape)
        # print("Object Quat shape before view:", space["object_quat"].shape)
        # print("Robot Base Pos shape before view:", space["robot_base_pos"].shape)
        # print("Robot Base Quat shape before view:", space["robot_base_quat"].shape)
        # print("Taken Actions shape before view:", space["taken_actions"].shape)
        # print("!"*20)
        # print("Commands shape:", space["commands"].view(states.shape[0], -1).shape)
        # print("DOF Diff shape:", space["dof_diff"].view(states.shape[0], -1).shape)
        # print("DOF Vel shape:", space["dof_vel"].view(states.shape[0], -1).shape)
        # print("Object Pos shape:", space["object_pos"].view(states.shape[0], -1).shape)
        # print("Object Quat shape:", space["object_quat"].view(states.shape[0], -1).shape)
        # print("Robot Base Pos shape:", space["robot_base_pos"].view(states.shape[0], -1).shape)
        # print("Robot Base Quat shape:", space["robot_base_quat"].view(states.shape[0], -1).shape)
        # print("Taken Actions shape:", space["taken_actions"].view(states.shape[0], -1).shape)
        # print("!"*20)
        ...
        mean_actions = self.action_net(combined_input)
        
        return mean_actions, self.log_std_parameter, {}


class MultiModalCritic(DeterministicMixin, Model):
    """Enhanced critic with RGB-guided depth processing"""
    
    def __init__(self, observation_space, action_space, device,
                 dropout_rate=0.2, clip_actions=False):
        ...
        other_state_dim = 85
        total_input_dim = 64 + other_state_dim + self.num_actions
        # total_input_dim=163
    ...
    def compute(self, inputs, role):
        ...
        # Process multimodal inputs (same as actor)
        rgb_images = space['front_cloud'].permute(0, 3, 1, 2).float()
        
        if 'depth_image' in space:
            depth_images = space['depth_image'].permute(0, 3, 1, 2).float()
        else:
            depth_images = rgb_images.mean(dim=1, keepdim=True)
            
        foreground_masks = self.fg_extractor(rgb_images)
        depth_features = self.depth_extractor(depth_images, foreground_masks)
        ...
        # Combine features with actions
        combined_input = torch.cat([depth_features, other_states, actions], dim=-1)
...
# Updated training script integration
def create_multimodal_models(env, device):
    """Create multimodal SAC models with foreground-masked depth processing"""
    models_sac = {}
    
    models_sac["policy"] = MultiModalStochasticActor(
        env.observation_space, env.action_space, device, 
        clip_actions=True
    )
    
    models_sac["critic_1"] = MultiModalCritic(
        env.observation_space, env.action_space, device,
        
    )
    
    models_sac["critic_2"] = MultiModalCritic(
        env.observation_space, env.action_space, device, 
        
    )
    
    models_sac["target_critic_1"] = MultiModalCritic(
        env.observation_space, env.action_space, device,
        
    )
    
    models_sac["target_critic_2"] = MultiModalCritic(
        env.observation_space, env.action_space, device,
        
    )
    
    # Initialize parameters with smaller std for pretrained components
    for name, model in models_sac.items():
        if hasattr(model, 'fg_extractor'):
            # Don't reinitialize the frozen foreground extractor
            pass
...
# Replace the model instantiation section in go2_train.py with:
models_sac = create_multimodal_models(env, device)

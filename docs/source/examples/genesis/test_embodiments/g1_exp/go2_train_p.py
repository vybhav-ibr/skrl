...
class Shared(GaussianMixin, DeterministicMixin, Model):
    def __init__(self, observation_space, action_space, device, clip_actions=False,
        ...
        # self.alter_net = nn.Sequential(nn.Linear(self.num_observations, 512),
        #                     nn.ELU(),
        #                     nn.Linear(512, 256),
        #                     nn.ELU(),
        #                     nn.Linear(256, 128),
    ...
    def compute(self, inputs, role):
        ...
        if role == "policy":
            # self._shared_output = self.alter_net(inputs["states"])
            self._shared_output = self.net(inputs["states"])
            return self.mean_layer(self._shared_output), self.log_std_parameter, {}
        elif role == "value":
            # shared_output = self.alter_net(inputs["states"]) if self._shared_output is None else self._shared_output
